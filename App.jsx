...
function Assistant({ t, lang, plan, checked, resources }) {
  ...
  const sendMessage = async () => { if (!input.trim()) return; const userMessage = { role: "user", text: input.trim() }; setMessages((current) => [...current, userMessage]); setInput(""); let response;
...
try {
  ...
  response = data.reply || data.message || data.text;
} catch (error) {
  response = "TEST: fallback is running because /api/assistant was not reached.";
